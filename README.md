...
| 3(c) | 여러 개선안 제안, baseline과 작은 예제로 비교, 최종 후보 선택 | 완료 | 보강 중 | `docs/24_problem_3_method_portfolio.md`, `scripts/run_problem_3_hamiltonian_variant_candidates.py`, `scripts/summarize_problem_3_method_portfolio.py` | actor-critic만 단독 결론으로 쓰지 않는다. Hamiltonian+random mixture, Hamiltonian two-way, hybrid, actor-critic을 후보 포트폴리오로 비교한다. |
...
| Problem 3 extension | Validated main candidate + method portfolio added | 한지후 | continuous를 main 3(a/b) result로 두고 hybrid와 actor-critic을 3(c) 후보로 비교 |
