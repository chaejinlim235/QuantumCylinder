...
기본값은 cycle 사이 대기 없이 바로 다음 cycle을 시작하며, `-MaxCycles 0`은 `Ctrl+C`로 멈출 때까지 계속 실행한다는 뜻입니다. 진행 상태는 `results/continuous_problem_3/latest_status.md`, 변화 기록은 `results/continuous_problem_3/progress_log.md`, 상세 로그는 `logs/continuous_problem_3/`에 남습니다. 일부러 쉬게 만들고 싶을 때만 `-CycleMinutes 30`처럼 값을 줍니다.
