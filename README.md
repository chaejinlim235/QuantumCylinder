...
| 3(c) | 개선안 제안, baseline과 작은 예제로 비교 | 완료 | 보강 중 | `docs/22_overnight_problem_3_evidence_handoff.md`, `docs/23_problem_3_actor_critic_handoff.md`, `scripts/run_problem_3_actor_critic_denoising.py` | actor-critic은 target-aware policy search로 제한해 서술하고 최종 숫자 source of truth를 하나로 고정 |
...
| Problem 3 extension | Validated main candidate + actor-critic candidate added | 한지후 | actor-critic을 main 3(c) 후보로 쓸지 appendix로 둘지 팀 검수 |
| Problem 3 overnight evidence | Harvested | 한지후, 김승빈 | `docs/22_overnight_problem_3_evidence_handoff.md` 기준으로 보고서/발표 반영 |
| Problem 3 physical interpretation | In progress | 김승빈 | measurement-induced denoising proxy와 target-aware actor-critic claim 범위 검수 |
...
| --- | --- | --- | --- |
| #6 | 임채진 | Open | ipynb 최종 보고서 작성 및 actor-critic 3(c) 반영 여부 결정 |
| #8 | 김승빈 | Open | 물리적 해석, actor-critic claim 범위, figure/table 패키징 |
| #19 | 김건우 | Open | 코드/Qiskit 구현 해석 및 actor-critic non-unitary filter consistency 검수 |
| #31 | 한지후 | Open | Hermes feedback loop 운영, actor-critic 검증, 최종 gatekeeping |
...
3. **Problem 3 - Further extension**
   ...
   - target-aware actor-critic policy search 기반 3(c) 개선 후보
