...
| 3(c) | 여러 개선안 제안, baseline과 작은 예제로 비교, 최종 후보 선택 | 완료 | 보강 중 | `docs/24_problem_3_method_portfolio.md`, `scripts/summarize_problem_3_method_portfolio.py`, `docs/23_problem_3_actor_critic_handoff.md` | actor-critic만 단독 결론으로 쓰지 않는다. continuous, axis-only, hybrid, actor-critic, collapse diagnostic을 같은 포트폴리오 표로 비교한다. |
...
3. **Problem 3 - Further extension**
   ...
   - hybrid 1M+1F toy와 target-aware actor-critic policy search를 포함한 3(c) 후보 포트폴리오
...
| 3 method portfolio | 여러 Problem 3 후보를 한 표로 비교 | `scripts/summarize_problem_3_method_portfolio.py` | `results/problem_3_baseline_collapse_defense/`, `results/problem_3_hybrid_diffusion_toy/`, `results/problem_3_actor_critic_denoising/` |
