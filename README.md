...
Open `solution/solution_1.ipynb` first. The USB-ready package is under
`submission/usb_package/` and contains both `presentation/` and `source_code/`.
Final figures and tables are under `solution/figures/` and `solution/tables/`,
with the concise final guide in `solution/README.md`.

Optional IBM QPU validation is documented in `docs/IBM_QPU_VALIDATION.md`.
...
| Problem 3 actor-critic 개선안 handoff | `docs/23_problem_3_actor_critic_handoff.md` |
...
| 3(b) | diffusion setting controlled modification 및 trade-off 분석 | 완료 | 보강 반영 | `docs/26_problem_3b_to_3c_storyline.md`, `results/problem_3_seed_sweep/seed_sweep_summary.md`, `results/problem_3_frozen_parameter_holdout/frozen_holdout_summary.md` | 수치 나열이 아니라 seed robustness, 거리 개선, 작은 axis-only margin, diversity/success trade-off를 명확히 분석 |
| 3(c) | 3-b 분석 기반 two-way projected denoising 제안, baseline과 작은 예제로 비교 | 완료 | 멘토 검수 필요 | `docs/26_problem_3b_to_3c_storyline.md`, `docs/24_problem_3_method_portfolio.md`, `docs/25_mentor_feedback_brief.md`, `scripts/run_problem_3_hamiltonian_variant_candidates.py`, `scripts/summarize_problem_3_method_portfolio.py` | 3-b에서 드러난 recoverability trade-off를 바탕으로 Hamiltonian two-way post-selection을 본문 main으로 제안한다. Hamiltonian+random final kick, hybrid, actor-critic은 appendix/ablation으로 내린다. axis-only는 baseline, continuous post-selection은 3-b control/reference로 설명한다. 최종 notebook 사본은 `solution/solution_1.ipynb`. |
...
| Problem 3 extension | Validated 3-b analysis + two-way 3-c main updated | 한지후 | 3-b에서 continuous control의 이득과 작은 axis-only margin을 분석하고, 그 trade-off에서 Hamiltonian two-way post-selection을 3-c main으로 도출 |
| Problem 3 overnight evidence | Harvested | 한지후, 김승빈 | `docs/22_overnight_problem_3_evidence_handoff.md` 기준으로 보고서/발표 반영 |
| Problem 3 physical interpretation | In progress | 김승빈 | measurement-induced denoising proxy와 후보별 claim 범위 검수 |
...
| --- | --- | --- | --- |
| #6 | 임채진 | Open | ipynb 최종 보고서 작성 및 Problem 3 two-way storyline 반영 |
| #8 | 김승빈 | Open | 물리적 해석, 후보별 claim 범위, figure/table 패키징 |
| #19 | 김건우 | Open | 코드/Qiskit 구현 해석 및 후보별 non-unitary filter consistency 검수 |
| #31 | 한지후 | Open | Hermes feedback loop 운영, two-way storyline 검증, 최종 gatekeeping |
...
3. **Problem 3 - Further extension**
   ...
   - `Z/X/Y` axis-only baseline과 continuous projection basis control을 이용한 3-b trade-off 분석
   - 3-b에서 드러난 거리 개선, diversity retention, post-selection success probability trade-off를 바탕으로 Hamiltonian two-way post-selection을 3(c) main 개선안으로 제안
   - Hamiltonian+random mixture, hybrid 1M+1F toy, target-aware actor-critic policy search는 appendix/ablation 후보로 분리
...
현재 Problem 3는 20-seed sweep에서 `20/20 use_as_main`이며, main result claim이 가능한 상태입니다. 다만 axis-only 대비 median score margin이 `0.010000`으로 크고 명확한 우위는 아니므로, 최종 보고서에서는 3-b를 recoverability trade-off 분석으로 제시하고 3-c 후보를 그 분석에서 도출합니다.
...
| --- | --- | --- | --- |
| final solution | 심사자용 최종 notebook과 안정 산출물 | `solution/solution_1.ipynb` | `solution/figures/`, `solution/tables/` |
...
| 3 Hamiltonian variants | Hamiltonian+random mixture, Hamiltonian two-way 후보 | `scripts/run_problem_3_hamiltonian_variant_candidates.py` | `src/quantum_cylinder/problem_3_continuous_projected_denoising.py` |
| 3 actor-critic | target-aware RL-style denoising 후보 | `scripts/run_problem_3_actor_critic_denoising.py` | `src/quantum_cylinder/problem_3_actor_critic_denoising.py` |
...
| 3 two-way + appendix portfolio | 3-c main two-way와 appendix/ablation 후보를 한 표로 비교 | `scripts/summarize_problem_3_method_portfolio.py` | `results/problem_3_seed_sweep/`, `results/problem_3_hamiltonian_variants/`, `results/problem_3_hybrid_diffusion_toy/`, `results/problem_3_actor_critic_denoising/` |
...
|   |-- 23_problem_3_actor_critic_handoff.md
|   |-- experiments/
...
|   |-- run_problem_3_actor_critic_denoising.py
|   |-- run_problem_3_continuous_denoising.py
|   |-- run_problem_3_finalist_autopilot.ps1
|   |-- run_problem_3_hamiltonian_variant_candidates.py
...
|   |-- summarize_problem_3_method_portfolio.py
