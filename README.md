...
3일 전체 로드맵과 현재 진행도는 `docs/16_three_day_roadmap.md`에서 관리합니다.
...
| #31 | 한지후 | Open | Hermes feedback loop 운영, PR/check 관리, 최종 gatekeeping |
...
Problem 1/2 코드를 처음 읽는 순서는 `docs/17_problem_1_2_code_reading_guide.md`에 정리합니다.
...
심사위원에게 보여줄 정량 평가 증거를 강화하는 계획은 `docs/15_quantitative_evaluation_plan.md`에 정리합니다.
