...
| 3(c) | 여러 개선안 제안, baseline과 작은 예제로 비교, 최종 후보 선택 | 완료 | 멘토 검수 필요 | `docs/24_problem_3_method_portfolio.md`, `docs/25_mentor_feedback_brief.md`, `scripts/run_problem_3_hamiltonian_variant_candidates.py`, `scripts/summarize_problem_3_method_portfolio.py` | actor-critic만 단독 결론으로 쓰지 않는다. Hamiltonian+random mixture, Hamiltonian two-way, hybrid, actor-critic을 후보 포트폴리오로 비교한다. 최종 notebook 사본은 `C:\Users\sky_m\Downloads\QuantumCylinder_final_submission_report_problem3c_variants_v5.ipynb`. |
...
| Problem 3 extension | Validated main candidate + method portfolio added | 한지후 | continuous를 main 3(a/b) result로 두고 Hamiltonian+random mixture, Hamiltonian two-way, hybrid, actor-critic을 3(c) 후보로 비교 |
...
3. **Problem 3 - Further extension**
   ...
   - Hamiltonian+random mixture, Hamiltonian two-way, hybrid 1M+1F toy, target-aware actor-critic policy search를 포함한 3(c) 후보 포트폴리오
...
| 3 method portfolio | 여러 Problem 3 후보를 한 표로 비교 | `scripts/summarize_problem_3_method_portfolio.py` | `results/problem_3_seed_sweep/`, `results/problem_3_hamiltonian_variants/`, `results/problem_3_hybrid_diffusion_toy/`, `results/problem_3_actor_critic_denoising/` |
