...
QuantumCylinder는 random-unitary diffusion과 Hamiltonian projected diffusion을 같은 fidelity 기반 metric으로 재현하고, continuous measurement-basis post-selection을 통해 quantum diffusion의 recoverability, success probability, diversity retention, control/resource cost trade-off를 정량화한 small-scale benchmark를 제안한다.
...
> 좋은 diffusion은 단순히 `S0`에서 멀리 퍼지는지만으로 평가할 수 없다. 다시 회복 가능한지, post-selection 성공확률이 충분한지, 다양성을 보존하는지, 제어 비용이 납득 가능한지를 함께 봐야 한다.
...
2. Frozen-parameter holdout으로 selection bias 방어
3. Identity, axis-only, continuous, collapse baseline 비교표 생성
4. Strong-scrambling 또는 angle-scale ablation
5. 최종 보고서/발표에 들어갈 figure, table, claim, limitation 생성
6. 실패한 후보는 main result를 대체하지 않고 appendix/fallback으로 기록
