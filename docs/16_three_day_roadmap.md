...
Problem 1과 Problem 2는 baseline 구현과 diagnostic 출력이 준비된 상태다. 현재 단기 목표는 이 결과를 팀원이 읽을 수 있는 ipynb 형태로 정리해 Problem 1/2를 완전히 닫는 것이다. Problem 3는 continuous projected denoising 후보가 현재 seed sweep 기준 main result로 사용할 수 있는 상태다.

다만 최종 제출 전까지는 다음 세 가지가 반드시 보강되어야 한다.

1. Problem 1/2가 문제 요구사항을 어떻게 만족하는지 ipynb에 정성 설명과 실행 결과를 붙인다.
...
| Problem 1/2 ipynb | 오늘의 active priority | `docs/18_problem_1_2_notebook_completion_plan.md`, `양자정보경진대회_v*.ipynb` | 전원 |
...
| 22:00-24:00 | Problem 1/2 ipynb v1 작성 및 코드 직접 리뷰 | `양자정보경진대회_v1.ipynb`, review notes | 전원 |

### Day 2 - 2026-06-30

| Time | Goal | Output | Owner |
| --- | --- | --- | --- |
| 00:00-09:00 | Problem 1/2 ipynb v2/v3 보완, Problem 3 automation은 별도 유지 | `양자정보경진대회_v2.ipynb`, `v3` 후보 | 전원 |
...
2. `docs/18_problem_1_2_notebook_completion_plan.md` 기준으로 원본 노트북을 덮어쓰지 않고 `양자정보경진대회_v1.ipynb`, `v2`, `v3` 사본을 만든다.
3. `.\scripts\invoke_hermes_task.ps1 final-sync-fix -Yolo -MaxTurns 360`으로 팀원 변경사항 반영, test, pipeline 점검을 반복한다.
4. 김건우는 Problem 2 Hamiltonian term, qubit order, projection basis 설명을 검증한다.
5. 임채진은 Problem 1의 cluster/scrambling 설명과 Problem 3 limitation 문장을 정리한다.
6. 김승빈은 generated `results/`에서 보고서에 넣을 figure/table 후보를 재현 명령과 함께 정리한다.
7. 한지후는 PR/check/merge gate를 관리하고, 최종 제출용 코드 경로를 단순하게 유지한다.
...
| Problem 1/2 notebook | 원본을 덮어쓰지 않고 `v1`, `v2`, `v3` 사본으로 저장 |
