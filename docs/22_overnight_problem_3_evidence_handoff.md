...
현재 최종 보고서의 3-b/3-c 스토리라인은 `docs/26_problem_3b_to_3c_storyline.md`를 우선한다. 이 문서는 evidence의 원천이고, 3-c 작성은 3-b 분석에서 two-way projected denoising을 본문 main으로 도출하는 방식으로 업데이트한다.
...
2. axis-only projection을 discrete baseline으로 두고 continuous basis control의 이득과 한계를 비교한다.
...
보고서 연결: 이 숫자는 3-b에서 "continuous basis가 압도적으로 좋다"는 결론이 아니라 recoverability trade-off 분석으로 쓴다. 3-c 본문 main은 이 trade-off를 더 강하게 활용하는 two-way projected denoising으로 제시한다.
...
### Day 2 Actor-Critic Target-Aware Candidate

| Metric | Value |
| --- | --- |
| Method | actor-critic policy search over `K_lambda = diag(1, lambda, lambda, lambda)` |
| Evaluated rows | `30` |
| Seeds | `1..10` |
| Input steps | `[1, 2, 3]` |
| Beats 3(a) continuous baseline on MMD | `30 / 30` |
| Beats 3(a) continuous baseline on Wasserstein | `30 / 30` |
...
해석: raw target ensemble을 reward 계산에 사용하므로 일반적인 unknown-target denoiser가 아니라 target-aware policy selection으로 제한한다. 최종 본문 main이 아니라 appendix/upper-bound 후보로 분리한다.
...
| Actor-critic handoff | `docs/23_problem_3_actor_critic_handoff.md` |
...
| Owner | Next action |
| --- | --- |
| 한지후 | 최종 notebook/report에 3-b trade-off와 3-c two-way main hierarchy를 반영하고, actor-critic은 appendix 후보로 분리한다. |
| 김승빈 | support worker 결과에서 figure/table 후보를 고르고, 재현 로그 위치를 발표자료에 연결한다. |
| 김건우 | Hamiltonian/projection/post-selection 해석 문장, actor-critic `K_lambda` filter 해석, "왜 auxiliary qubit post-selection이 조건부 상태를 만든다"는 설명을 검수한다. |
| 임채진 | Problem 1/2 정성 설명과 Problem 3 limitation, 3-c two-way main 및 appendix 후보 분리를 보고서 흐름에 자연스럽게 배치한다. |
