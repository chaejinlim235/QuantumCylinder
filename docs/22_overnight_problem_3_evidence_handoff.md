...
해석: Problem 3(c)의 강한 새 후보로 쓸 수 있다. 다만 raw target ensemble을 reward 계산에 사용하므로 일반적인 unknown-target denoiser가 아니라 target-aware policy selection으로 제한한다. 최종 보고서에 넣을 경우 continuous post-selection main result를 대체하기보다, "target prior를 사용할 수 있을 때 actor-critic으로 filter strength를 선택하면 더 강한 toy denoising improvement가 가능했다"는 식으로 분리한다.
...
| Owner | Next action |
| --- | --- |
| 한지후 | 최종 notebook/report에 위 hierarchy와 actor-critic 후보 숫자를 반영하고, 최종 source of truth를 하나로 고정한다. |
...
| 임채진 | Problem 1/2 정성 설명과 Problem 3 limitation, actor-critic 3(c) 반영 여부를 보고서 흐름에 자연스럽게 배치한다. |
