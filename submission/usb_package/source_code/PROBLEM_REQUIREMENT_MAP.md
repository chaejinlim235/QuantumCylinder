...
| 1(c) | Show random-unitary diffusion trajectory and distance curves. | `solution/figures/fig2_random_unitary_haar_baseline.png`, `solution/tables/problem1_haar_reference.csv` | `scripts/create_solution_haar_baseline.py`, `scripts/run_problem_1_2_baselines.py` | Random-unitary diffusion rapidly reaches a strong-scrambling, Haar-like distance plateau; the Haar mean/band is emphasized and zoomed in the final figure. | Complete |
...
| 2(d) | Discuss resource/control-cost proxies. | `solution/figures/fig_metric_aligned_comparison_readable.png`, `solution/tables/problem_2d_resource_matches.csv` | `scripts/run_problem_1_2_baselines.py`, `scripts/create_readable_problem2_figures.py` | We compare random layer/entangler count against fixed-Hamiltonian evolution time \(t\) and projection-basis control. | Complete |
...
| 3(b) | Analyze a controlled modification and trade-off. | `solution/tables/problem3b_measurement_basis_tradeoff.csv`, `solution/solution_1.ipynb` | `scripts/summarize_problem_3_seed_sweep.py` | Measurement-basis angle \(\beta\) controls the effective non-unitary map, so \(\Delta D\) must be read together with \(p_{\mathrm{succ}}\) and \(R_{\mathrm{div}}\). | Complete |
